...
//             photoURL: user.photoURL || user.avatarUrl || `${API_BASE_URL.replace('/api', '')}/avatars/userLogo.png`,
//         };
//     }, [user, mongoUser]);
...
// // REMOVE THIS LOCAL CustomUser INTERFACE:
// // interface CustomUser {
// //     uid: string;
// //     mongoUserId: string;
// //     name: string;
...
//             try {
//                 if (!user || !mongoUser) {
//                     throw new Error('User not authenticated or profile data not loaded.');
//                 }
...
//                 const response = await fetch(`${API_BASE_URL}/users/profile/${mongoUser._id}`, { // Ensure correct user ID is used
//                     method: 'PUT',
//                     headers: {
//                         'Authorization': `Bearer ${token}`,
//                         // Note: 'Content-Type': 'multipart/form-data' is automatically set by the browser
//                         // when using FormData, including the correct boundary. Do NOT set it manually here.
//                     },
//                     body: formData,
//                 });
...
//                     // The `responseData` object will contain the error from the backend.
//                     const errorMessage = responseData.message || 'Failed to update profile.';
//                     throw new Error(errorMessage);
//                 }

//                 // If response.ok is true, responseData contains the successful JSON payload (updated user data)
//                 const updatedUser = responseData;

//                 // Update local state with the new avatar URL and clear preview/file
//                 setCurrentAvatarUrl(updatedUser.avatarUrl); // This should directly be the full URL from the backend
//                 setNewAvatarFile(null);
//                 setNewAvatarPreview(null); // Clear preview once saved

//                 // Display success toast
//                 toast.success('Profile updated successfully!');

//                 // Crucial: Update the user state in your AuthContext
//                 // This ensures that `user` and `mongoUser` in your context are up-to-date
//                 // across your application, reflecting the new name, bio, and avatarUrl.
//                 // if (refreshUser) { // Assuming refreshUser is a function passed from AuthContext
//                 //     refreshUser(updatedUser); // Pass the newly updated user data back to the context
//                 // }
...
//         return {
//             _id: mongoUser._id, // Add the required _id property from mongoUser
//             uid: user.uid,
//             mongoUserId: mongoUser._id, // Use _id from mongoUser
//             name: user.name || user.displayName || 'Unknown User',
//             email: user.email || '',
...
//             avatarUrl: user.avatarUrl || `${API_BASE_URL.replace('/api', '')}/avatars/userLogo.png`,
//             photoURL: user.photoURL || user.avatarUrl || `${API_BASE_URL.replace('/api', '')}/avatars/userLogo.png`,
//             firebaseUid: user.uid // Ensure firebaseUid is also present if your CustomUser in types.ts needs it
//         } as CustomUser; // Explicitly cast to CustomUser to satisfy TypeScript
//     }, [user, mongoUser]);
...
// // Ensure this Post interface is consistent if also used elsewhere
// interface Post {
//     _id: string;
//     text: string;
//     mediaUrl?: string;
...
//             name: mongoUser.name || user.displayName || 'Unknown User', // Use mongoUser.name if available
//             avatarUrl: mongoUser.avatarUrl || user.photoURL || `${API_BASE_URL.replace('/api', '')}/avatars/userLogo.png`,
//             _id: mongoUser._id, // Add _id from mongoUser
//             email: user.email, // Add email from user
//             displayName: user.displayName, // Add displayName from user
//             photoURL: user.photoURL, // Add photoURL from user
//         };
//     }, [user, mongoUser]);
