...
//     callUser: (receiver: User) => void;
//     answerCall: () => void;
//     endCall: () => void;
//     incomingCall: { from: User; signal: any; callId: string } | null;
//     callAccepted: boolean;
//     // stream: MediaStream | null; // Remove this if local stream is handled by ref within provider
//     myVideoRef: React.RefObject<HTMLVideoElement>; // Expose local video ref
//     peerVideoRef: React.RefObject<HTMLVideoElement>; // Expose peer video ref
//     isCalling: boolean;
//     outgoingCallTarget: User | null;
//     currentCallId: string | null;
//     isCallActive: boolean; // Add this derived state
//     remoteStream: MediaStream | null; // Expose remoteStream
//     localStream: MediaStream | null; // Expose localStream for toggling tracks
// }
...
//         incomingCall,
//         callAccepted,
//         stream: localStreamRef.current, // Expose the actual local stream
//         remoteStream: peerVideoRef.current?.srcObject as MediaStream || null, // Expose the remote stream
//         myVideoRef,
//         peerVideoRef,
//         isCalling,
//         outgoingCallTarget,
//         currentCallId,
//         isCallActive: isCalling && callAccepted, // Derived state
//         localStream: localStreamRef.current, // Provide access to the actual local stream
//     };
...
export const CallProvider = ({
  ...
  const leaveCall = () => {
    if (!socketRef.current) return;
    socketRef.current.emit("endCall", { to: currentUserId });
    endCallCleanup();
  };
  ...
  const startCall = (userId: string, type: "audio" | "video") => {
    callUser(userId, type === "video");
  };
