...
interface MessageListProps {
    ...
    onEditMessage: (message: Message) => void;
    editingMessageId: string | null;
    backgroundImageUrl: string | null; // New prop for background image
}
...
const MessageList: FC<MessageListProps> = ({
    ...
    onEditMessage,
    editingMessageId,
    backgroundImageUrl, // Destructure new prop
}) => {
    const messagesEndRef = useRef<HTMLDivElement>(null);

    useEffect(() => {
        messagesEndRef.current?.scrollIntoView({ behavior: 'smooth' });
    }, [messages, loading]); // Scroll to the latest message whenever messages change
    ...
    const messageListStyle: React.CSSProperties = {
        ...
        backgroundColor: backgroundImageUrl ? 'transparent' : '#f9fafb', // Fallback to gray-50 if no image
    };
