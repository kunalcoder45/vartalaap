...
//   useEffect(() => {
//     // Step 1: Connect socket if not connected
//     if (!socket.connected) {
//       socket.auth = { userId: currentUserId };
//       socket.connect();
//     }

//     // Step 2: Check current online users
//     fetch('http://localhost:5000/api/online-users') // change to your socket server
//       .then((res) => res.json())
//       .then((onlineUserIds) => {
//         setIsChatUserOnline(onlineUserIds.includes(chatUser._id));
//       })
//       .catch((err) => console.error('Failed to fetch online users:', err));

//     // Step 3: Listen to real-time status changes
...
const ChatHeader: FC<ChatHeaderProps> = ({
  ...
  useEffect(() => {
    ...
    setIsChatUserOnline(onlineUsers.has(chatUser._id));
